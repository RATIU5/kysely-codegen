...
import type { PostgresBunDB } from './postgres-bun-db';

export type PostgresBunDomainInspector = {
  rootType: string;
  typeName: string;
  typeSchema: string;
};
...
export type PostgresBunIntrospectorOptions = {
  defaultSchemas?: string[];
  domains?: boolean;
  partitions?: boolean;
};

export class PostgresBunIntrospector extends Introspector<PostgresBunDB> {
  protected readonly options: PostgresBunIntrospectorOptions;

  constructor(options?: PostgresBunIntrospectorOptions) {
    super();
  ...
  async introspect(options: IntrospectOptions<PostgresBunDB>) {
  ...
  async introspectDomains(db: Kysely<PostgresBunDB>) {
    if (!this.options.domains) {
      return [];
    }

    const result = await sql<PostgresBunDomainInspector>`
  ...
  async introspectEnums(db: Kysely<PostgresBunDB>) {
